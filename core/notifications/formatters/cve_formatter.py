...
def build_cve_telegram_message(cve: Any) -> str:
    ...
    # Severidade (emoji de criticidade mantido)
    severity_map = {"CRITICAL": "🔴 CRITICAL", "HIGH": "🟠 HIGH", "MEDIUM": "🟡 MEDIUM", "LOW": "🟢 LOW"}
    sev_label = severity_map.get(cve.risk_tag, cve.risk_tag)

    msg = f"🚨 <b>{headline}</b>\n"
    msg += "━━━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"<b>CVE:</b> {cve_id}\n"
    msg += f"<b>Risk:</b> {sev_label} (CVSS {cvss})\n"
    msg += f"<b>Vendor:</b> {vendor}\n"
    msg += f"<b>Product:</b> {product}\n"

    if hasattr(cve, "cwes") and cve.cwes:
        msg += f"<b>CWE:</b> {', '.join(html.escape(c) for c in cve.cwes)}\n"
    if hasattr(cve, "threats") and cve.threats:
        msg += f"<b>Threats:</b> {', '.join(html.escape(t) for t in cve.threats)}\n"
    ...
    if len(paragraphs) >= 2:
        msg += f"<b>[ Summary ]</b>\n{paragraphs[0]}\n\n"
        msg += f"<b>[ Technical Impact ]</b>\n{paragraphs[1]}\n\n"
    else:
        msg += f"<b>[ Description ]</b>\n{description}\n\n"

    if cve.impacted_clients:
        msg += f"<b>Affected Assets:</b> <code>{', '.join(cve.impacted_clients)}</code>\n\n"

    links: list[str] = []
    if url:
        links.append(f'🔗 <a href="{url}">NVD Detail</a>')
    advisory_url = getattr(cve, "advisory_url", None)
    if advisory_url:
        links.append(f'🔗 <a href="{html.escape(advisory_url)}">{vendor} Advisory</a>')
    if links:
        msg += "\n".join(links)
